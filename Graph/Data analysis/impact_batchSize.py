...
proposed_igpr = (np.array([20,35,68,152]))
...
ax.bar(ind,dummy,width,color=[(1,0.7,0)],edgecolor='black',label="Per-batch latency")
# ax.bar(ind,dummy,width,color='lime',edgecolor='black',label="CSIR")
ax.bar(ind,dummy,width,color='darkgrey',edgecolor='black',label="Worst-case latency")
...
static_igpr = (np.array([455,395,379,425]))
...
leg = plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.55), handler_map={matplotlib.lines.Line2D: SymHandler()}, 
            fontsize='46', ncol=1, handleheight=1, labelspacing=0.5, frameon=False) 
