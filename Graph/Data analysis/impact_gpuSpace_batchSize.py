...
proposed_igpr = (np.array([390,400,370,360]))
...
ax.bar(ind,dummy,width,color=[(1,0.7,0)],edgecolor='black',label="25% GPU Space")
# ax.bar(ind,dummy,width,color='lime',edgecolor='black',label="CSIR")
ax.bar(ind,dummy,width,color='darkgrey',edgecolor='black',label="50% GPU Space")
ax.bar(ind,dummy,width,color='tomato',edgecolor='black',label="75% GPU Space")
...
ax.bar(ind,dummy,width,color='deepskyblue',edgecolor='black',label="100% GPU Space")
...
static_igpr = (np.array([456,398,405,408]))
...
time_varying_igpr = (np.array([454,397,380,386]))
...
centralized_igpr = np.array([455,395,379,425])
...
ax.set_position([box.x0, box.y0 + box.height*0.06, box.width, box.height*0.70])
#bbox_to_anchor=(0.5, 1.6)
leg = plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.55), handler_map={matplotlib.lines.Line2D: SymHandler()}, 
            fontsize='46', ncol=2, handleheight=1, labelspacing=0.0, frameon=False) 
