...
ax.plot(stepX,proposed,c='red',marker="x",mew=4,markersize=26,ls='-',label="AdaInf",fillstyle='full', linewidth = 4)

centralized = [95,95,95,95.3,95,95,95,95,95.4,95,95,95,95,95.5,95,95,95,95.8,95,95]
ax.plot(stepX,centralized,c='black',marker="*",markersize=28,ls='-',label="Ekya",fillstyle='full', linewidth = 4)
...
ax.plot(stepX, static, c = 'blue', marker = 'o', markersize = 26, ls = '-', label = "Scrooge", fillstyle = 'full', linewidth = 4)
...
ax.set_ylabel('Finish ratio')
ax.set_xlabel('Inference window (sec)')
# ax.set_ylim(0,4500)
# ax.set_xticks(ind+4*width)
#ax.set_xticklabels(vals['error_type'].tolist())

# xvalues = [5, 10, 15, 20, 25, 30]
xvalues = [50,100,150,200,250,300,350,400,450,500,550,600,650,700,750,800,850,900,950,1000]
# plt.xticks(xvalues)
plt.xticks(xvalues, fontsize =35, rotation = 45)
...
ax.set_position([box.x0, box.y0 + box.height*0.09, box.width, box.height*0.75])
