...
proposed_igpr = (np.array([17.9, 20.6, 46.4, 46.4]))
...
ax.bar(ind,proposed_igpr,width,color=[(1,0.7,0)],edgecolor='black')
...
ax.bar(ind,dummy,width,color=[(1,0.7,0)],edgecolor='black',label="Retraining")
# ax.bar(ind,dummy,width,color='lime',edgecolor='black',label="CSIR")
ax.bar(ind,dummy,width,color='darkgrey',edgecolor='black',label="Inference")
# ax.bar(ind,dummy,width,color='tomato',edgecolor='black',label="TVGR")
...
# ax.bar(ind,dummy,width,color='deepskyblue',edgecolor='black',label="CSIR")
...
static_igpr = (np.array([32.54, 72.8, 52.42, 52.42]))
...
ax.bar(ind+1*width, static_igpr, width, color = 'darkgrey', edgecolor = 'black')
...
# time_varying_igpr = (np.array(vals_igpr['time_varying'].tolist()))
...
# xvalues = [20,40,60,80,100]
# plt.xticks(xvalues)
# plt.xticks(xvalues)
ax.set_xticklabels(['AdaInf', 'Ekya', 'Scrooge', 'Scrooge*'])
...
ax.set_position([box.x0 + box.width*0.06, box.y0 + box.height*0.06, box.width, box.height*0.70])
#bbox_to_anchor=(0.5, 1.6)
leg = plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.30), handler_map={matplotlib.lines.Line2D: SymHandler()}, 
            fontsize='48', ncol=2, handleheight=1.5, labelspacing=0.6, frameon=False) 
